...
import com.example.wayvo.dtos.request.TrajetRequest;
...
public class TrajetService {
        ...
        for (Reservation reservation : passagers) {
            ...
            if (token != null && !token.isBlank()) {
                notificationService.envoyerNotificationAvecLien(
                    ...
                    " est en route. Appuyez pour voir la position.",
                    lienMaps
                );
            }
        }
    ...
    public Page<TrajetResponse> rechercher(String villeDepart, String villeArrivee,
                                            LocalDate dateDepart, Pageable pageable) {
        ...
        List<TrajetResponse> resultats = tousLesTrajets.stream()
            ...
            // ✅ Filtre sur la date si fournie
            .filter(t -> {
                if (dateDepart != null) {
                    return t.getDateHeureDepart().toLocalDate().equals(dateDepart);
                }
                return true;
            })
            .filter(t -> {
