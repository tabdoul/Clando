...
public class ReservationService {
    ...
    public ReservationResponse creer(ReservationRequest request) {
        ...
        notificationService.envoyerNotification(
            trajet.getConducteur().getExpoPushToken(),
    ...
    public ReservationResponse repondreNegociation(Long reservationId, boolean accepter) {
        ...
        if (accepter) {
            ...
            notificationService.envoyerNotification(
                reservation.getPassager().getExpoPushToken(),
                "Réservation confirmée ! ✅",
                "Votre réservation pour " +
                reservation.getTrajet().getVilleDepart() + " → " +
                reservation.getTrajet().getVilleArrivee() + " est confirmée !"
            );
        } else {
            if (reservation.getNbTentatives() >= 1) {
                ...
                // Notifier le passager refus définitif
                notificationService.envoyerNotification(
                    reservation.getPassager().getExpoPushToken(),
                    "Réservation refusée ❌",
                    "Votre réservation pour " +
                    reservation.getTrajet().getVilleDepart() + " → " +
                    reservation.getTrajet().getVilleArrivee() + " a été refusée."
                );
            } else {
                reservation.setStatut(Reservation.StatutReservation.PRIX_REFUSE);
                reservation.setNbTentatives(reservation.getNbTentatives() + 1);

                // Notifier le passager prix refusé
                notificationService.envoyerNotification(
                    reservation.getPassager().getExpoPushToken(),
                    "Prix refusé 💬",
                    "Le conducteur a refusé votre prix pour " +
                    reservation.getTrajet().getVilleDepart() + " → " +
                    reservation.getTrajet().getVilleArrivee() +
                    ". Vous pouvez faire une nouvelle proposition."
                );
            }
        }
    ...
    public ReservationResponse nouvelleProposition(Long reservationId, Double nouveauPrix) {
        ...
        // Notifier le conducteur nouvelle proposition
        notificationService.envoyerNotification(
            reservation.getTrajet().getConducteur().getExpoPushToken(),
            "Nouvelle proposition de prix 💰",
            reservation.getPassager().getPrenom() + " propose " +
            nouveauPrix.longValue() + " GNF pour " +
            reservation.getTrajet().getVilleDepart() + " → " +
            reservation.getTrajet().getVilleArrivee()
        );
    ...
    public Map<String, Object> annuler(Long id) {
        ...
        notificationService.envoyerNotification(
            trajet.getConducteur().getExpoPushToken(),
            "Réservation annulée ❌",
            reservation.getPassager().getPrenom() + " a annulé sa réservation pour " +
            trajet.getVilleDepart() + " → " + trajet.getVilleArrivee()
        );
